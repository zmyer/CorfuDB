...
public class SequencerView extends AbstractView {
    ...
    public TokenResponse nextToken(Set<UUID> streamIDs, int numTokens) {
        return layoutHelper(l -> CFUtils.getUninterruptibly(l.getPrimarySequencer()
    ...
    public TokenResponse nextToken(Set<UUID> streamIDs, int numTokens,
        ...
        return layoutHelper(l -> CFUtils.getUninterruptibly(l.getPrimarySequencer().nextToken(
                streamIDs, numTokens, conflictInfo)));
    }

    public void trimCache(long address) {
        getCurrentLayout().getPrimarySequencer().trimCache(address);
    }

    /**
     * Dumps the sequencer state.
     */
