 ...
 */
public class SequencerServerTest extends AbstractServerTest<SequencerServer> {
    ...
    public void responseForEachRequest() {
        for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L, Collections.<UUID>emptySet(),
                            false, false)));
    ...
    public void tokensAreIncreasing() {
        ...
        for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L, Collections.<UUID>emptySet(),
                            false, false)));
            long thisToken = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
        ...
        for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(0L, Collections.<UUID>emptySet(),
                            false, false)));
            long checkToken = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
        ...
        for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            ...
                            false, false)));
            long thisTokenA = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(0L,
                            Collections.singleton(streamA),
                            false, false)));
            long checkTokenA = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            ...
                            false, false)));
            long thisTokenB = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(0L,
                            Collections.singleton(streamB),
                            false, false)));
            long checkTokenB = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(0L,
                            Collections.singleton(streamA),
                            false, false)));
            long checkTokenA2 = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
    ...
    public void checkBackpointersWork() {
        ...
        for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            Collections.singleton(streamA), false,
                            false)));
            long checkTokenA = getLastPayloadMessageAs(TokenResponse.class)
                    .getBackpointerMap().get(streamA);
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(MULTI_TOKEN,
                            Collections.singleton(streamA),
                            false, false)));
            thisTokenA = getLastPayloadMessageAs(TokenResponse.class)
                    .getToken();
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            Collections.singleton(streamA),
                            false, false)));
            checkTokenA = getLastPayloadMessageAs(TokenResponse.class)
                    .getBackpointerMap().get(streamA);

            assertThat(thisTokenA + MULTI_TOKEN - 1)
                    .isEqualTo(checkTokenA);

            // check the requesting multiple tokens does not break
            // the back-pointer for the multi-entry
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            Collections.singleton(streamA),
                            false, false)));
        ...
        for (int i = 0; i < PARAMETERS.NUM_ITERATIONS_LOW; i++) {
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            ...
                            false, false)));
            long thisTokenA = getLastPayloadMessageAs(TokenResponse.class)
                    .getStreamAddresses().get(streamA);
            ...
            sendMessage(new CorfuPayloadMsg<>(CorfuMsgType.TOKEN_REQUEST,
                    new TokenRequest(1L,
                            ...
                            false, false)));
            long thisTokenB = getLastPayloadMessageAs(TokenResponse.class)
                    .getStreamAddresses().get(streamB);
